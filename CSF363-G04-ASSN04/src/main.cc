...
int main(int argc, char *argv[]) {
	...
	FILE *newSource = fopen("debugFile.be", "w+");
	...
	extern std::string returnString(int token, std::string);
	...
	while(true){
		...
		std::string writeStr = returnString(token, nwtext);
		fwrite(writeStr.data(), sizeof(char),writeStr.size() , newSource);
	}
	std::string writeStr = "\n\0";
	fwrite(writeStr.data(), sizeof(char),writeStr.size() , newSource);
	...
	newSource = fopen("debugFile.be", "r");
